    ...
    <section class="cart">
        ...
        <div class="cart-content row">
            ...
            <div class="cart-content-left">
                ...
                // Thực hiện truy vấn SQL để lấy thông tin sản phẩm từ tbl_product dựa trên product_id trong tbl_order_details
                $sql = "SELECT tbl_product.*, tbl_order_details.*
                FROM tbl_product
                ...
                // Kiểm tra xem có bất kỳ dòng dữ liệu nào được trả về không
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<tr>';
                        echo '<td><img src="/NTKQ/admin/uploadss/' . $row['product_img'] . '"></td>';
                        echo '<td>' . $row['product_name'] . '</td>';
                        echo '<td>' . $row['si_ze'] . '</td>';
                        echo '<td>' . $row['quantity'] . '</td>';
                        echo '<td>' . number_format($row['price'], 0, ',', '.') . 'đ</td>';

                        echo '<td><a class="delete-link" href="delete_order.php?order_id=' . $row['order_id'] . '">X</a></td>';
                        echo '</tr>';
                    }

                    echo '</table>';
                } else {
                    echo '<h5 style="color: red; font-weight: bold;margin-top: 100px; margin-bottom: 50px; text-align: center;">Quý khách chưa có sản phẩm trong giỏ hàng.</h5>';
            ...
            <div class="cart-content-right">
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <p><?php echo number_format($totalPrice1, 0, ',', '.') . 'đ'; ?></p>
                        </td>
                    </tr>
                    <tr>
