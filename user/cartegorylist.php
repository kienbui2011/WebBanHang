...
if (isset($_GET['Search'])) {
    ...
    $sql_timkiem = "SELECT * FROM tbl_cartegory WHERE cartegory_name LIKE '%" . $tukhoa . "%' ";
    $query_timkiem = mysqli_query($conn, $sql_timkiem);
    $isSearch = true; // Đã thực hiện tìm kiếm
} else {
    $tukhoa = '';
    $sql_danhsachbandau = "SELECT * FROM tbl_cartegory LIMIT 10";
...
<div class="admin-content-right">
    <div class="admin-content-right-cartegory-list">
        ...
        <div class="additional">
            <form action="cartegorylist.php" method="GET" name="search">
                <input type="text" name="timkiem" />
                <input type="submit" name="Search" value="Tìm kiếm" />
            </form>

            <button><a href="cartegoryadd.php">Thêm</a></button>
        </div>

        <?php if ($isSearch) : ?>
            <button><a href="cartegorylist.php">Thoát</a></button>
        <?php endif; ?>
